pipeline {
    ...
    stages {
        stage ('SCM') {
            steps {
                git 'https://github.com/C1-80670-Kiran/assignment7.git'
            }
        }
        stage ('docker login') {
            steps {
        ...
        stage ('docker build image') {
            steps {
                sh '/usr/bin/docker build -t kirandada0303723/myhttpd .'
            }
        }
        stage ('docker push image') {
            steps {
               sh '/usr/bin/docker image push kirandada0303723/myhttpd'
            }
        }
